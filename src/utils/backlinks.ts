...
interface BacklinkData {
  [targetPath: string]: Link[];
}

/**
 * Extract internal links from markdown content
 */
function extractInternalLinks(content: string, sourcePath: string, sourceTitle: string, sourceType: 'blog' | 'project'): Link[] {
  const links: Link[] = [];
 ...
 */
export function buildBacklinks(): BacklinkData {
 ...
 */
export function generateBacklinksHTML(pagePath: string, backlinksData: BacklinkData): string {
 ...
 */
export function getBacklinksCSS(): string {
  return `
.backlinks {
  margin-top: 3rem;
  padding-top: 2rem;
